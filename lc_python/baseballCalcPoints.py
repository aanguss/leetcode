...
class Solution:
    def calPoints(self, ops: List[str]) -> int:
        ...
        for i in range(len(ops)):
            if debug: print(ops[i])
            if ops[i] == 'D':
                updatedOps.append(int(updatedOps[-1]) * 2)
            elif ops[i] == 'C':
                updatedOps.pop()
            elif ops[i] == '+':
                updatedOps.append(int(updatedOps[-1]) + int(updatedOps[-2]))
            else:
                updatedOps.append(int(ops[i]))
            if debug: print(updatedOps)
        totalScore = sum(updatedOps)
