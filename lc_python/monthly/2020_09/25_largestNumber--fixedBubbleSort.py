    ...
    def largestNumber(self, nums: List[int]) -> str:
        ...
        for i in range(len(nums)):
            for j in range(i + 1, len(nums)):
                stringIJ = nums[i] + nums[j]
                stringJI = nums[j] + nums[i]
                # if debug: print("%s vs %s -- %s" % (nums[i], nums[j], nums))
                if debug: print("%s vs %s -- %s" % (stringIJ, stringJI, nums))
                if stringIJ < stringJI:
                    temp = nums[j]
                    nums[j] = nums[i]
                    nums[i] = temp
        ...
        # BUBBLE SORT NOT REALLY WORKING FOR THIS
        ...
        #                     if nums[i][currentDigit - 1] < nums[i][currentDigit - 1]:
        ...
        for num in nums:
            if debug: print(num)
            largestNum += str(num)
        if len(largestNum) > 1 and largestNum[0][0] == '0':
            if debug: print('found largest == 0 --', largestNum[0][0])
            largestNum = '0'
        return largestNum
